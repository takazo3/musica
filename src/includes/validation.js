    ...
    configure({
      generateMessage:(ctx) => {
        const messages = {
          ...
          alpha_spaces:`The field ${ctx.field} may only contain alphabetical and spaces.`,
          ...
          country_excluded:`Due to restrictions, we do noe accept users from this location.`,
          passwords_mismatch:`The password don't match.`,
          tos:`You must accept the Terms of Service.`

        };
        const message = messages[ctx.rule.name] ? messages[ctx.rule.name]:`The field ${ctx.field}`;
        return message;
      },
      validateOnBlur: true,
